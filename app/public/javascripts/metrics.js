...
$(document).ready(function() {
    ...
    if(lastChecked === undefined || new Date().getTime() - JSON.parse(lastChecked) > 1000*60*60*24) {
        endTime = new Date();
        startTime = new Date(2015, 0, 12);
        data = getMetrics(startTime.getTime(), endTime.getTime());
        localStorage.metricsData = JSON.stringify(data);
        localStorage.metricsTimeStamp = new Date().getTime();
    }
    else {
        console.log("Cached");
    }
    data = JSON.parse(localStorage.metricsData);
    mostCommonIds(data);
