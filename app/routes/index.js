...
router.post('/getmetrics', function (req, res) {
    ...
    db.collection('metrics').find({timestamp: {$gt : parseInt(req.body.startTime), $lt : parseInt(req.body.endTime)}}).toArray(function (err, items) {
        latestMetrics = items
...
router.post('/setbulletin', function(req, res) {
    if(req.session.loggedIn) {
        ...
        res.send({msg: 'Announcement changed'})
    }
    else {
        res.send({msg:'Nice try'})
    }
